...
model {
  ...
  {  // vectorized format of sampling distribution
     ...
     // link
     for(n in 1:N) {
       p[n] = inv_cloglog( xl[n]*lambda + Qast[n,indexf]*thetaf + Qast[n,indexr]*(theta_random[jj[n]]) ); 
       
       // + Qast[n]*(theta_random[jj[n]]) + log(H[n])
     }
...
generated quantities {
  ...
  //for(n in 1:N) {
  // log_lik[n] = bernoulli_lpmf( c[n] | inv_cloglog(xl[n]*lambda + Qast[n,indexf]*thetaf + Qast[n,indexr]*(theta_random[jj[n]])) );
  //}
}
